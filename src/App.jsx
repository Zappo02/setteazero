...
const ACCEPTS = { GK:["GK"], DF:["DF"], MF:["MF","DF","FW"], FW:["FW","MF"] };
const DIFFICULTY = {
...
function genMatchEvents(myXI, oppXI, myName, oppName, myStr, oppStr, rng){
  ...
  const myXG=Math.max(.3,1.15+Math.max(-1,Math.min(2.4,diff/6)));
  const opXG=Math.max(.3,1.15+Math.max(-1,Math.min(2.4,-diff/6)));
...
function buildTournament(squad,formation,seed){
  ...
  for(const round of rounds){
    ...
    ties.push({round,opp:opp.team,year:opp.year,oppStr,...m});
    if(!m.won)break;
  }
  const champion=ties.length===rounds.length&&ties.every(t=>t.won);
  const fin=ties[ties.length-1];
  const sevenZero=champion&&fin.gf>=7&&fin.ga===0;
  // statistiche giocatori (solo partite giocate, vinte o no)
  ...
  return {ties,champion,sevenZero,myStr,stats:Object.values(stats)};
}
...
function SlotMachine({spinning,result,onSpin,canSpin,roundLabel,locked}){
...
export default function App(){
  ...
  const start=()=>{ setSquad(new Array(totalSlots).fill(null)); setCurrent(null); setSim(null); setPhase("draft"); };

  const doSpin=()=>{
    if(spinning)return; setSpinning(true); setCurrent(null);
  ...
  return(
    <div className="wc-app">
      <style>{CSS}</style>
      <Header/>
      {phase==="setup"&&<Setup {...{formationKey,setFormationKey,difficulty,setDifficulty,nationLock,setNationLock,start}}/>}
      {phase==="draft"&&<Draft {...{formation,squad,hide,round,totalSlots,spinning,current,doSpin,openSlots,choose,nationLock}}/>}
...
function Setup({formationKey,setFormationKey,difficulty,setDifficulty,nationLock,setNationLock,start}){
    ...
    <div className="panel">
      <div className="field-block">
        ...
        <div className="chips">{Object.keys(FORMATIONS).map(k=>(
          <button key={k} className={`chip${formationKey===k?" on":""}`} onClick={()=>setFormationKey(k)}>{k}</button>))}
        </div>
      </div>
      <div className="field-block">
...
function Draft({formation,squad,hide,round,totalSlots,spinning,current,doSpin,openSlots,choose,nationLock}){
...
function LiveTournament({sim,squad,formation,hide,onDone}){
  ...
  useEffect(()=>{
    if(minute>=90){
      const t=setTimeout(()=>{
        if(tieIdx<sim.ties.length-1 && tie.won) setTieIdx(tieIdx+1);
    ...
    <div className="live">
      ...
      <div className="live-grid">
        ...
        <div className="live-feed">
          ...
          {minute>=90&&(
            <div className={`feed-final ${tie.won?"won":"lost"}`}>
              Fine. {gf}–{ga}{tie.pens&&` · rigori ${tie.pens}`} — {tie.won?"passi il turno":"eliminato"}
            </div>
          )}
        </div>
      </div>
...
function Result({squad,formation,hide,sim,onRestart,onRebuild}){
  ...
  const sub=sim.sevenZero?"Hai chiuso la finale 7–0. Impresa leggendaria.":sim.champion?"Coppa alzata — ma non è finita 7–0. Riprova.":`Fuori ai ${last.round}.`;
  ...
  const shareText=useMemo(()=>{
    const line=sim.ties.map(m=>`${m.round}: ${m.gf}-${m.ga}${m.pens?` (r ${m.pens})`:""}`).join(" · ");
    const mvp=topScorers[0];
    return `Sette a Zero ⚽ ${headline}\n${line}\nMVP ${mvp.player.n} (${mvp.goals}G ${mvp.assists}A) · Forza XI ${sim.myStr}\nuniversosportivo.com`;
  },[sim,headline]);
  ...
  return(
    <div className="result">
      ...
      <div className="result-grid">
        ...
        <div>
          <h3 className="sec">Cammino</h3>
          <div className="bracket">{sim.ties.map((m,i)=>(
      ...
      <div className="result-actions">
        <Brass onClick={()=>{navigator.clipboard?.writeText(shareText);setCopied(true);setTimeout(()=>setCopied(false),1600);}}>{copied?"Copiato!":"Copia risultato"}</Brass>
