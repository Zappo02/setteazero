...
async function shareResult({squad,formation,sim,headline,scoreline,topScorers}){
  ...
  const brass="#f2cd6b", neon="#36f0bf", neonPink="#ff4d92", neonRed="#ff6157", txt="#eaf4ec", mute="#8fb8a3";
  // ---- SFONDO PIATTO UNIFORME (un solo colore, niente gradienti né bande)
  x.fillStyle="#0a2418";x.fillRect(0,0,W,H);
  const isWin=sim.champion||sim.sevenZero;
  const accent=sim.sevenZero?neonPink:sim.champion?neon:!sim.qualified?neonRed:brass;

  const PAD=60;
  let cy=140; // cursore verticale: ogni blocco avanza da qui

  // ---- BRAND
  cy=neonLine(x,"SETTE A ZERO",W/2,cy,W-100,88,brass);
  x.textAlign="center";x.fillStyle=mute;x.font="600 32px Arial";
  cy+=14;x.fillText("build · spin · 7–0",W/2,cy);cy+=40;

  // ---- ESITO (headline + scoreline)
  cy+=30;
  cy=neonLine(x,headline.toUpperCase(),W/2,cy,W-90,100,accent);
  if(scoreline){ cy+=14; cy=neonLine(x,scoreline,W/2,cy,W-90,108,accent); }
  cy+=30;

  // ---- CAMPO (altezza calcolata per riempire fino ai box bassi)
  const bottomBlock=360;       // spazio per cammino + box in fondo (aumentato per non sovrapporre)
  const footer=70;
  const fieldTop=cy;
  const fieldBottom=H-footer-bottomBlock;
  const fh=Math.max(640,fieldBottom-fieldTop);
  drawPitch(x,PAD,fieldTop,W-PAD*2,fh,squad,formation);
  cy=fieldTop+fh+50;

  // ---- CAMMINO (una riga, centrata, sopra i box)
  x.textAlign="center";x.fillStyle=mute;x.font="700 25px Arial";
  x.fillText("IL CAMMINO",W/2,cy);cy+=42;
  ...
  let camm=`Gironi ${gW}V-${gD}N-${gL}P`;
  if(ko.length) camm+="  ·  "+ko.map(m=>`${m.round.slice(0,3)} ${m.gf}-${m.ga}`).join("  ·  ");
  x.fillStyle=txt;x.font="600 28px Arial";
  wrapText(x,camm,W/2,cy,W-90,38,"center");

  // ---- BOX MVP + FORZA (ancorati in basso)
  const boxH=176, boxGap=24;
  const by=H-footer-boxH;
  const totW=W-PAD*2-boxGap, mvpW=Math.round(totW*0.63), forW=totW-mvpW;
  const mvp=topScorers[0];
  // MVP
  fillRGBA(x,neon,.10);roundRect(x,PAD,by,mvpW,boxH,22);x.fill();
  x.strokeStyle=neon;x.lineWidth=3;roundRect(x,PAD,by,mvpW,boxH,22);x.stroke();
  x.textAlign="left";
  x.fillStyle=neon;x.font="800 28px Arial";x.fillText("★ MIGLIORE IN CAMPO",PAD+30,by+50);
  x.fillStyle="#fff";x.font="900 56px Arial Black,Arial";
  x.fillText(fitName(x,mvp.player.n,mvpW-60,56),PAD+30,by+112);
  x.fillStyle=mute;x.font="500 30px Arial";
  x.fillText(`${mvp.team} '${mvp.year.slice(2)} · ${mvp.goals}G ${mvp.assists}A`,PAD+30,by+154);
  // FORZA
  const fbx=PAD+mvpW+boxGap;
  fillRGBA(x,brass,.12);roundRect(x,fbx,by,forW,boxH,22);x.fill();
  x.strokeStyle=brass;x.lineWidth=3;roundRect(x,fbx,by,forW,boxH,22);x.stroke();
  x.textAlign="center";const fcx=fbx+forW/2;
  x.fillStyle=brass;x.font="900 96px Arial Black,Arial";x.fillText(String(sim.myStr),fcx,by+108);
  x.fillStyle=mute;x.font="700 25px Arial";x.fillText("FORZA XI",fcx,by+150);

  // ---- FOOTER
  x.textAlign="center";x.fillStyle=brass;x.font="800 38px Arial";
  x.fillText("universosportivo.com",W/2,H-30);
...
function fillRGBA(x,hex,a){const c=hexToRgb(hex);x.fillStyle=`rgba(${c.r},${c.g},${c.b},${a})`;}
// disegna una o più righe di testo neon centrate a partire da y (top), restituisce la y dopo l'ultima riga
function neonLine(x,text,cx,topY,maxW,size,color){
  ...
  let y=topY+size; // baseline prima riga
  for(const l of lines){ neonGlyph(x,l,cx,y,color,size); y+=size*1.04; }
  return y-size*0.20;
}
// neon pulito: glow morbido + riempimento netto, NIENTE stroke sbavato
function neonGlyph(x,t,cx,y,color,size){
  x.save();x.textAlign="center";x.font=`900 ${size}px Arial Black,Arial`;
  x.shadowColor=color;x.shadowBlur=size*0.28;
  x.fillStyle=color;x.fillText(t,cx,y);          // base colorata con glow
  x.shadowBlur=0;
  x.fillStyle="#ffffff";                          // cuore bianco netto
  x.globalAlpha=0.92;x.fillText(t,cx,y);x.globalAlpha=1;
  x.restore();
}
function fitName(x,name,maxW,size){
  x.font=`900 ${size}px Arial Black,Arial`;
  if(x.measureText(name).width<=maxW)return name;
  const parts=name.split(" ");const last=parts[parts.length-1];
  return x.measureText(last).width<=maxW?last:name;
}
...
function drawPitch(x,X,Y,w,h,squad,formation){
  ...
  // card giocatori: dimensione DINAMICA in base alla distanza minima tra slot,
  // così non si sovrappongono mai (qualunque formazione)
  let minDist=Infinity;
  for(let i=0;i<formation.length;i++)for(let j=i+1;j<formation.length;j++){
    const dx=(formation[i][1]-formation[j][1])/100*w;
    const dy=(formation[i][2]-formation[j][2])/100*h;
    minDist=Math.min(minDist,Math.hypot(dx,dy));
  }
  const bw=Math.max(120,Math.min(200,minDist*0.96));
  const bh=Math.max(64,Math.min(88,minDist*0.46));
  formation.forEach(([role,px,py],i)=>{
    const f=squad[i];if(!f)return;
    const cx=X+(px/100)*w,cy=Y+(py/100)*h;
    const L=cx-bw/2, T=cy-bh/2;
    x.fillStyle="rgba(5,16,11,.95)";roundRect(x,L,T,bw,bh,12);x.fill();
    x.strokeStyle="#f2cd6b";x.lineWidth=2.5;roundRect(x,L,T,bw,bh,12);x.stroke();
    // puntino bandiera in ALTO-CENTRO, sopra il nome
    x.fillStyle=hslStr(teamHue(f.team));
    x.beginPath();x.arc(cx,T+bh*0.20,Math.max(5,bh*0.085),0,7);x.fill();
    // nome centrato
    const nameSize=Math.round(Math.min(28,bh*0.32));
    x.fillStyle="#fff";x.font=`800 ${nameSize}px Arial`;x.textAlign="center";
    const nm=fitName(x,f.player.n,bw-20,nameSize);
    x.fillText(nm,cx,T+bh*0.60,bw-16);
    // anno · rating
    const subSize=Math.round(Math.min(22,bh*0.25));
    x.fillStyle="#8fd9bd";x.font=`600 ${subSize}px Arial`;
    x.fillText(`${f.year} · ${f.player.r}`,cx,T+bh*0.86,bw-16);
  });
}
...
function Setup({formationKey,setFormationKey,difficulty,setDifficulty,rerollMode,setRerollMode,nationLock,setNationLock,start}){
  ...
  return(
    <div className="panel">
      ...
      <div className="rules">
        <p><b>Come si gioca.</b> Tiri la leva: la slot abbina un <b>anno di Mondiale</b> a una <b>nazionale</b>. Se esce una <b>campione del mondo</b>, la slot lampeggia. Da quella rosa scegli <b>un solo giocatore</b> per uno slot libero compatibile. Completi l'undici e parte il <b>torneo live</b>, turno dopo turno: riesci a chiudere sul <b>7–0</b>? A fine torneo vedi le statistiche dei tuoi giocatori.</p>
      </div>

      <button className="info-toggle" onClick={()=>setShowInfo(s=>!s)}>
        {showInfo?"▾":"▸"} Come sono calcolati i rating?
      </button>
      {showInfo&&(
        <div className="info-box">
          <p>Il valore di ogni giocatore <b>non è un voto assoluto di bravura</b>, ma riflette <b>quanto è andata bene la sua nazionale in quel preciso Mondiale</b>, pesato così:</p>
          <p><b>60%</b> dal <b>piazzamento nel torneo</b> (vincere il Mondiale vale molto più che uscire ai gironi) e <b>40%</b> dal <b>prestigio storico</b> della nazionale. Dentro ogni rosa, poi, i titolari valgono più delle riserve.</p>
          <p className="info-ex">Qualche esempio concreto:</p>
          <ul className="info-list">
            <li><b>Pelé, Brasile 1970</b> → 92 — campione da assoluto protagonista</li>
            <li><b>Maradona, Argentina 1986</b> → 92 — Mondiale vinto da trascinatore</li>
            <li><b>Il miglior portiere del Qatar 2022</b> → 72 — eliminato ai gironi in casa</li>
            <li><b>La Corea del Sud 2002</b> → ~81 — semifinale a sorpresa premiata</li>
          </ul>
          <p className="info-note">È per questo che lo stesso campione può avere valori diversi in edizioni diverse, e perché una nazionale materasso resta sotto anche con i suoi uomini migliori.</p>
        </div>
      )}
      ...
      <div className="cross-links">
        <span className="cross-eyebrow">Dalla stessa squadra</span>
        <div className="cross-row">
          <a className="cross-card" href="https://universosportivo.com/38-0-serie-a/" target="_blank" rel="noopener">
            <b>38 a Zero</b><small>Costruisci la rosa perfetta di Serie A →</small></a>
          <a className="cross-card" href="https://universosportivo.com/quiz-calcio/" target="_blank" rel="noopener">
            <b>Quiz Calcio</b><small>Nuove sfide ogni 24 ore →</small></a>
        </div>
      </div>
    </div>
  );
...
function Draft({formation,squad,hide,round,totalSlots,spinning,current,doSpin,openSlots,choose,nationLock,rerollsUsed,maxRerolls}){
  ...
  const [showPitch,setShowPitch]=useState(false); // su mobile il campo è collassato di default
  useEffect(()=>{setPending(null);},[current]);
  const rerollsLeft=maxRerolls===Infinity?Infinity:Math.max(0,maxRerolls-rerollsUsed);
  // riepilogo slot liberi per ruolo (compatto, sempre visibile su mobile)
  const freeByRole={GK:0,DF:0,MF:0,FW:0};
  formation.forEach(([role],i)=>{if(!squad[i])freeByRole[role]++;});
  return(
    <div className="draft">
  ...
  return(
    <div className="live">
      <div className="live-scoreboard">
        <div className="live-round">{tie.round}</div>
...
function Result({squad,formation,hide,sim,onRestart,onRebuild}){
  ...
  return(
    <div className="result">
      ...
      <div className="cross-links">
        <span className="cross-eyebrow">Continua a giocare su universosportivo.com</span>
        <div className="cross-row">
          <a className="cross-card" href="https://universosportivo.com/38-0-serie-a/" target="_blank" rel="noopener">
            <b>38 a Zero</b><small>La rosa perfetta di Serie A →</small></a>
          <a className="cross-card" href="https://universosportivo.com/quiz-calcio/" target="_blank" rel="noopener">
            <b>Quiz Calcio</b><small>Nuove sfide ogni 24 ore →</small></a>
        </div>
      </div>
...
.info-toggle{align-self:flex-start;background:none;border:none;color:var(--brass-hi);cursor:pointer;font-size:15px;font-weight:600;padding:4px 0;text-align:left}
.info-toggle:hover{text-decoration:underline}
.info-box{background:rgba(61,242,192,.06);border:1px solid #2f7d6a;border-radius:12px;padding:18px 20px;font-size:14px;line-height:1.6;display:flex;flex-direction:column;gap:10px}
.info-box p{margin:0}
.info-box b{color:#7df2cf}
.info-ex{opacity:.7;font-size:12px;letter-spacing:1px;text-transform:uppercase;margin-top:4px!important}
.info-list{margin:0;padding-left:18px;display:flex;flex-direction:column;gap:6px}
.info-list li{font-size:14px}
.info-list b{color:#eaf4ec}
.info-note{opacity:.7;font-size:13px;font-style:italic}
.cross-links{margin-top:8px;display:flex;flex-direction:column;gap:12px}
.cross-eyebrow{font-size:11px;letter-spacing:3px;text-transform:uppercase;opacity:.55}
.cross-row{display:flex;flex-wrap:wrap;gap:14px}
.cross-card{flex:1;min-width:220px;display:flex;flex-direction:column;gap:4px;text-decoration:none;background:linear-gradient(180deg,#123a26,#0c2a1b);border:1px solid #2f7d52;border-radius:14px;padding:16px 18px;transition:.15s}
.cross-card:hover{border-color:var(--brass-hi);transform:translateY(-2px)}
.cross-card b{color:var(--brass-hi);font-size:18px}
.cross-card small{color:#9fc4ad;font-size:13px}
...
.draft{display:grid;grid-template-columns:300px 1fr 320px;gap:24px;align-items:start;max-width:1200px;margin:0 auto}
.draft-left{display:flex;flex-direction:column;gap:14px;align-items:center}
.role-summary{display:flex;gap:8px;flex-wrap:wrap;justify-content:center}
.role-summary span{font-size:12px;font-weight:600;padding:5px 10px;border-radius:8px;background:#143726;border:1px solid #23583c}
